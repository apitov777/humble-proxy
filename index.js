...
async function getToken() {
  ...
  tokenExpires = Date.now() + json.expires_in * 1000 - 60_000;
  return cachedToken;
}

/* -------- 3. Rota que devolve o token -------- */
...
/* -------- 5. Ranking de Montarias -------- */
app.get('/api/mounts', async (_, res) => {
  try {
    const token = await getToken();
    ...
    const results = await Promise.all(filtered.map(async m => {
      ...
      const [mountsData, mediaData] = await Promise.all([
        fetch(`${base}/collections/mounts?namespace=profile-us&locale=pt_BR`, {
          headers: { Authorization: `Bearer ${token}` }
        }).then(r => r.json()),
      ...
      const mounts = mountsData.mounts?.length || 0;
      const avatar = mediaData.assets?.find(a => a.key === 'avatar')?.value || '';

      return { name, mounts, avatar };
    }));

    results.sort((a, b) => b.mounts - a.mounts);
    res.json(results);
  } catch (err) {
    res.status(500).json({ error: 'mounts_failed', detail: err.message });
  }
});
